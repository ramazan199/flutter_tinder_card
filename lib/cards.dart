...
class CardStack extends StatefulWidget {
  ...
  final Function(Decision?, Function) onSwipeCallback;
  final MatchEngine matchEngine;
  final ValueListenable<bool>? showOverlay;

  CardStack(
      {required this.matchEngine,
      required this.onSwipeCallback,
      required this.mySlideTo,
      this.showOverlay});
...
class _CardStackState extends State<CardStack> {
  ...
  void toggleShowOverlay() => setState(() {
  ...
  void _onSlideComplete(SlideDirection direction) {
    ...
    switch (direction) {
      case SlideDirection.left:
        currenMatch.nope();
        widget.onSwipeCallback(currenMatch.decision!, toggleShowOverlay);
        break;
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: (showOverlay && (widget.showOverlay?.value ?? false))
          ? <Widget>[
              if (widget.matchEngine.recomCount > 1)
                DraggableCard(
              ...
              if (widget.matchEngine.recomCount > 0)
                DraggableCard(
                  screenHeight: MediaQuery.of(context).size.height,
                  screenWidth: MediaQuery.of(context).size.width,
          ...
          : [
              if (widget.matchEngine.recomCount > 1) _buildBackCard(),
              if (widget.matchEngine.recomCount > 0) _buildFrontCard()
            ],
    );
  }
}
...
class _DraggableCardState extends State<DraggableCard>
      ...
      ..addStatusListener((AnimationStatus status) {
        if (status == AnimationStatus.completed) {
          setState(() {
            ...
            if (widget.onSlideComplete != null && slideOutDirection != null) {
  ...
  void didUpdateWidget(DraggableCard oldWidget) {
    super.didUpdateWidget(oldWidget);
    // print("xzxzx");
    if (widget.mySlideTo == true && check == false) {
      print(widget.mySlideTo);
      _slideRight();
      check = true;

      //remove ram tolko eto
      // matchEngine.currentMatch.like();
      // matchEngine.cycleMatch();

      // widget.mySlideTo = null;
    }
  ...
  @override
  Widget build(BuildContext context) {
    // mySlideTo = Provider.of<Match>(context);
